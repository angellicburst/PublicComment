...
function clickSubMenu(element) {
	...
	// 按指定地址加载主页面(iframe)
	$("#mainPage").attr("src",$("input[name=basepath]").val() + "/adList");
}
...
function clickSubMenu1(element) {
	...
	$(element).children().attr("class","on");
	// 按指定地址加载主页面(iframe)
	$("#mainPage").attr("src",$("input[name=basepath]").val() + "/busList");
}
...
function clickSubMenu2(element) {
	...
	$(element).children().attr("class","on");
	// 按指定地址加载主页面(iframe)
	$("#mainPage").attr("src",$("input[name=basepath]").val() + "/comList");
}
...
function clickSubMenu3(element) {
	...
	$(element).children().attr("class","on");
	// 按指定地址加载主页面(iframe)
	$("#mainPage").attr("src",$("input[name=basepath]").val() + "/orderList");
}
...
function clickSubMenu4(element) {
	...
	$(element).children().attr("class","on");
	// 按指定地址加载主页面(iframe)
	$("#mainPage").attr("src",$("input[name=basepath]").val() + "/report");
}
